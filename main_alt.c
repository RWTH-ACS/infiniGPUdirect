...
void testBandwidthServer(unsigned int memSize, char *peer_node, float * result)
{
    ...
    for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
    {
        ib_client_send(d_idata, 1, memSize, peer_node, true, &temp_time);
        time_s += temp_time;
    }
...
void testBandwidthClient(unsigned int memSize, char *peer_node, float *result)
{
    ...
    for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
    {
        ib_client_send(h_idata, 1, memSize, peer_node, false, &temp_time);
        time_s += temp_time;
    }
...
int main(int argc, char **argv)
{
    ...
    if (server) {
        ...
        printf("The average Device to Host bandwidth: %f GB/s\n", DtH);
...
//        printInfo();
        ...
        printf("The average Host to Device bandwidth: %f GB/s\n", HtD);
